...
static void simulated_keyboard(void)
{
  char key = getchar();
  if (key == 'w')
    spade_call_press(5); // map_move(map_get_first('p'),  0, -1);
  else if (key == 's')
    spade_call_press(7); // map_move(map_get_first('p'),  0,  1);
  else if (key == 'a')
    spade_call_press(6); // map_move(map_get_first('p'),  1,  0);
  else if (key == 'd')
    spade_call_press(8); // map_move(map_get_first('p'), -1,  0);
  else if (key == 'i')
    spade_call_press(12); // map_move(map_get_first('p'),  0, -1);
  else if (key == 'k')
    spade_call_press(14); // map_move(map_get_first('p'),  0,  1);
  else if (key == 'j')
    spade_call_press(13); // map_move(map_get_first('p'),  1,  0);
  else if (key == 'l')
    spade_call_press(15); // map_move(map_get_first('p'), -1,  0);
  else
    return;
...
void render_stats(Color *screen)
{
  ...
  if (jerry_get_memory_stats(&stats))
  {
    ...
    if (stats.peak_allocated_bytes > 200000)
      mem_color = color16(255, 255, 0); // yellow
    if (stats.allocated_bytes > 200000)
      mem_color = color16(255, 0, 0); // red
  }

  int bitmap_count = state->render->doodle_index_count;
  if (bitmap_count > peak_bitmap_count)
    peak_bitmap_count = bitmap_count;
  ...
  if (sprite_count > peak_sprite_count)
    peak_sprite_count = sprite_count;
  ...
  {
    if (mem[i] != '\0')
      render_char(screen, mem[i], mem_color, i * 8, SCREEN_SIZE_Y);
    if (bitmaps[i] != '\0')
      render_char(screen, bitmaps[i], color16(255, 255, 255), i * 8, SCREEN_SIZE_Y + 8);
    if (sprites[i] != '\0')
      render_char(screen, sprites[i], color16(255, 255, 255), i * 8, SCREEN_SIZE_Y + 16);
  }
}

char **split_into_test_cases(char *script, int *count) {
  char **test_cases = NULL;
  int cases_count = 0;
  char *start = script, *end;
  while ((end = strstr(start, TEST_DELIMITER))) {
    int len = end - start;
    test_cases = realloc(test_cases, sizeof(char *) * (cases_count + 1));
    test_cases[cases_count] = (char *)malloc(len + 1);
    strncpy(test_cases[cases_count], start, len);
    test_cases[cases_count][len] = '\0';
    cases_count++;
    start = end + strlen(TEST_DELIMITER);
  }
  if (*start) {
    int len = strlen(start);
    test_cases = realloc(test_cases, sizeof(char *) * (cases_count + 1));
    test_cases[cases_count] = (char *)malloc(len + 1);
    strcpy(test_cases[cases_count], start);
    cases_count++;
  }
  *count = cases_count;
  return test_cases;
...
char *read_in_script(const char *path, int *size) {
...
int main(int argc, char *argv[]) {
  ...
  char **test_cases = split_into_test_cases(script, &test_case_count);
  free(script);

  for (int i = 0; i < test_case_count; ++i) {
    printf("Running Test Case %d\n", i + 1);
    js_init(test_cases[i], strlen(test_cases[i]));
    Sleep(2000);
  }
